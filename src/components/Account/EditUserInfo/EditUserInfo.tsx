...
export default function EditUserInfo({
  ...
  const [handleProfilePicture, setHandleProfilePicture] = useState<File | null>(
    null
  );
  const [handleProfilePicturePreview, setHandleProfilePicturePreview] =
    useState<string>("");
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (
      handleUsername.length === 0 &&
      handleBirthdate.length === 0 &&
      handleEmail.length === 0 &&
      handlePassword.length === 0 &&
      ...
      if (handleEmail.length > 0) {
        axios
          .post(`http://${SVIP}:${SVPORT}/api/verify-email`, {
            email: handleEmail,
          })
          .then((response) => {
        ...
        <section className={styles.editUserInfoSection}>
          <form onSubmit={handleSubmit} className={styles.form}>
            ...
            <div className={styles.uploadImage}>
              <StyledImage
                src={handleProfilePicturePreview ? handleProfilePicturePreview : profilePicture}

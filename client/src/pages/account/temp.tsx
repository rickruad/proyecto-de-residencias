...
const useUpdateUserInfo = ({ id, email, password, username, birthdate, profilePicture, newEmail, newPassword, newUsername, newBirthdate, newProfilePicture }: updateUserInfoProps) => {
  ...
  return Server.useUpdateUser({ formData });
}

export default function Home() {
  const { id, email, password, username, birthdate, profilePicture, admin } = Server.GetCurrentUserInformation();
  const { length, ids, emails, passwords, usernames, birthdates, profilePictures, statuses, admins } = Server.GetAllUsersInformation();
  ...
  for (let i = 1; i < length; i++) {
    const editUserInfo = () => {
      setIsEditUserInfoShow(!isEditUserInfoShow);
      setTitleEditUserInfo(`Cambiando la información de ${AuxiliarFunctions.wordsToCapitalLetter({ text: usernames[i] })}`);
    ...
    accountsHTML.push(
      <div key={i} className={styles.account}>
        <div className={styles.userInfoAccount}>
          <h4>{`${AuxiliarFunctions.wordsToCapitalLetter({ text: usernames[i] })} (${emails[i]})`}</h4>
        </div>
  ...
  return <>
    ...
    <main className={styles.container}>
      <section className={styles.usernameSection}>
        <div>
          <h2 className={classNames(styles.previewWithoutImage, profilePicture ? styles.hidePreviewWithoutImage : styles.showPreviewWithoutImage)}>{AuxiliarFunctions.wordsToAcronym({ text: username })}</h2>
          <Image
            ...
            height='100'
          />
          <h1 className={styles.username}>{AuxiliarFunctions.wordsToCapitalLetter({ text: username })}</h1>
        </div>
      </section>
